...
+ To build a Docker Image (this will create an image with `bhitsdev/pcm:latest` tag):
...
A few additional [SQL files](https://github.com/bhits-dev/pcm/blob/master/pcm-db-sample) are provided with this project to populate the database with sample `purpose`, `consent attestation term` and `consent revocation term` contents.
...
+ Run as a Docker Container: `docker run -d bhitsdev/pcm:latest <additional program arguments>`
...
+ `docker run -d bhitsdev/pcm:latest --server.port=80 --spring.datasource.password=strongpassword`
...
+ `docker run -d -v "/path/on/dockerhost/ssl_keystore.keystore:/path/to/ssl_keystore.keystore" bhitsdev/pcm:latest --spring.profiles.active=ssl --server.ssl.key-store=/path/to/ssl_keystore.keystore --server.ssl.key-store-password=strongkeystorepassword`
+ In a `docker-compose.yml`, this can be provided as:
```yml
version: '2'
services:
...
Please use [GitHub Issues](https://github.com/bhits-dev/pcm/issues) page to report issues.
